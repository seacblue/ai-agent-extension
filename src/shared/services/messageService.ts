...
export class MessageService {
  ...
  public establishConnection(): chrome.runtime.Port | null {
    ...
    try {
      ...
      this.panelPort.onDisconnect.addListener(() => {
        ...
        if (chrome.runtime.lastError) {
          ...
          const retryDelay = Math.min(
            this.baseConnectionRetryDelay * Math.pow(2, this.connectionRetryCount - 1),
            this.maxConnectionRetryDelay
          )
          
          console.log(`尝试重连 #${this.connectionRetryCount} (延迟 ${retryDelay}ms)...`)
          setTimeout(() => {
      ...
      console.log(`建立连接发生错误，尝试重连 #${this.connectionRetryCount} (延迟 ${retryDelay}ms)...`)
      setTimeout(() => {
        this.establishConnection()
      }, retryDelay)
...
---`
   ...
   * 终止当前消息处理
   */
  public async terminateMessage(onSendingStateChange?: (isSending: boolean) => void): Promise<void> {
    onSendingStateChange?.(false)
    this.finishThinkingProcess()
   ...
   * 处理流式传输中断
   */
  public handleStreamingInterrupt(onSendingStateChange?: (isSending: boolean) => void): void {
    this.isStreaming = false
    onSendingStateChange?.(false)
   ...
   * 添加助手消息
   */
  public addAssistantMessage(content: string, status: 'success' | 'error' = 'success'): Message {
    const message: Message = {
      id: generateId(),
      type: 'ASSISTANT',
      content,
