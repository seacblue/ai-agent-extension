  ...
  public async handleQuestion(options: QuestionOptions): Promise<void> {
    ...
    try {
      ...
      if (incomingPanelPort) {
        ...
        currentPanelPort = chrome.runtime.connect({
      ...
      if (finalPrompt.length > 20000) {
        console.log(
          `警告：整体提示词长度 ${finalPrompt.length} 字符，可能接近 token 限制`
        );
        // 如果提示词太长，可以进一步精简或只保留最相关部分
        if (finalPrompt.length > 30000) {
          finalPrompt =
            finalPrompt.substring(0, 30000) +
            '...\n[提示词已被截断以避免token超限]';
        }
      }
