...
class ElementSelectorService {
    ...
    private getElementInfo(element: Element): ElementInfo {
        try {
            ...
            const childElements = Array.from(element.children).slice(0, 5).map(child => ({
    ...
    private getCssSelector(element: Element): string {
        ...
        if (element.className && typeof element.className === 'string' && element.className.trim() !== '') {
            const classes = element.className.trim().split(/\s+/).map(cls => `.${cls}`).join('')
    ...
    private getElementPath(element: Element): string {
        ...
        while (current && current.tagName !== 'HTML') {
            ...
            if (current.id) {
                ...
                const classes = current.className.trim().split(/\s+/).map(cls => `.${cls}`).join('')
                selector += classes
            }
