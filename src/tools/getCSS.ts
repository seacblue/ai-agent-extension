...
class CSSToolImpl implements Tool {
  name = 'getCSS';
  description = 'CSS 样式分析工具，获取元素的计算样式';
  keywords = ['CSS', '样式', 'computedStyle', '计算样式', '元素', '样式分析'];

  async execute(params: GetCSSOptions = {}): Promise<ToolResult> {
    try {
  ...
  // 获取目标元素
  private getTargetElement(params: GetCSSOptions): ElementInfo | null {
...
// 导出 CSS 分析器工具实例
export const CSSTool = new CSSToolImpl();
