...
class DOMToolImpl implements Tool {
  ...
  async execute(params: any = {}, context?: ToolContext): Promise<ToolResult> {
    try {
      ...
      // 并行获取所有数据
      const [domStructure, pageHTML, semanticAnalysis] =
      ...
      const pageData = {
        ...
        domStructure:
          domStructure.status === 'fulfilled' ? domStructure.value : null,
        pageHTML: pageHTML.status === 'fulfilled' ? pageHTML.value : null,
        semanticAnalysis:
          semanticAnalysis.status === 'fulfilled'
  ...
  // DOM 结构获取函数
  private async getDOMStructure(
    options: DOMOptions = {}
  ): Promise<DOMStructure> {
    try {
      const {
        includeStyles = false,
  ...
  private async analyzeSemantics(): Promise<SemanticInfo> {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      console.error('语义化分析失败: ', errorMessage);
      // 返回默认的语义化信息
      return {
        hasHeader: false,
        hasNav: false,
